...
# Aggregate subrepo docs (Marchyo, Jotain, jstack, Makework) into local directories
# Requires devenv shell
aggregate-docs
```

Deployment is automatic via the Mintlify GitHub app when pushing to the `docs` branch. The `docs` branch is built by CI from `main` using devenv's `aggregate-docs` script.
...
- `marchyo/`, `jotain/`, `jstack/`, `makework/` — generated by `aggregate-docs`, not tracked in git
